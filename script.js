...
const i18n = {
  en: {
    ...
    "dashboard.refreshLatestData": "Refresh Latest Data",
    "dashboard.refreshing": "Refreshing latest sensor data...",
    "dashboard.refreshed": "Latest data refreshed at {time}.",
  ...
  zh: {
    ...
    "dashboard.refreshLatestData": "刷新最新数据",
    "dashboard.refreshing": "正在刷新最新传感器数据...",
    "dashboard.refreshed": "最新数据已在 {time} 刷新。",
...
function setLanguage(lang) {
  ...
  if (document.body.dataset.page !== "index") {
    ...
    if (refreshMessage && refreshMessage.dataset.refreshTime) {
      refreshMessage.textContent = t("dashboard.refreshed", {
        time: refreshMessage.dataset.refreshTime
      });
    }
...
function refreshLatestData() {
  ...
  if (refreshMessage) {
    refreshMessage.textContent = t("dashboard.refreshing");
    refreshMessage.dataset.refreshTime = "";
  }

  setTimeout(() => {
    const currentTime = formatCurrentTime();

    const possibleMotionStatus = ["detecting", "idle", "activityDetected", "bedExit", "noActivity"];
    const newMotionStatus = pickRandom(possibleMotionStatus);

    let newBedStatus = appState.dashboard.bedStatus;

    if (newMotionStatus === "bedExit") {
      newBedStatus = "unoccupied";
    } else if (newMotionStatus === "activityDetected" || newMotionStatus === "detecting") {
      newBedStatus = "occupied";
    } else {
      newBedStatus = pickRandom(["occupied", "unoccupied"]);
    }

    appState.dashboard = {
      motionStatus: newMotionStatus === "activityDetected" || newMotionStatus === "bedExit" || newMotionStatus === "noActivity"
        ? "detecting"
        : newMotionStatus,
      temperature: `${getRandomInteger(22, 27)}°C`,
      humidity: `${getRandomInteger(45, 65)}%`,
      lightStatus: appState.settings.lightToggle ? "on" : "off",
      bedStatus: newBedStatus,
      lastUpdate: currentTime
    };

    const newLogStatus = newMotionStatus === "detecting" ? "activityDetected" : newMotionStatus;

    appState.logs.push({
      time: currentTime,
      status: newLogStatus,
      location: newLogStatus === "bedExit" ? "bed" : pickRandom(["bedside", "bedroom"])
    });
    ...
    if (refreshMessage) {
      refreshMessage.dataset.refreshTime = currentTime;
      refreshMessage.textContent = t("dashboard.refreshed", { time: currentTime });
    }
  }, 600);
}
